...
from .decorators import allowed_users, unauthenticated_user, admin_only, allow_subscribe_user
...
def customerPage(request):
    context = {}
    data = subscribeData(request)
    context = {'subscriber':data['subscriber'], 'user':data['user'], 'winner_requirement': data['winner_visit_number']}
    return render(request, 'accounts/customerPage.html', context)


def subscribeData(request):
    winner_visit_number = 10 # Hard Code winner coount
    if request.user.is_authenticated:
        user = request.user
        subscriber, created = Subscriber.objects.get_or_create(user=user)
        subscriber.visitCount+=1 #increment every time user visit this page

        if subscriber.subscribe == True and subscriber.visitCount >= winner_visit_number:
            subscriber.winner = True
        else:
            subscriber.winner = False
        subscriber.save()
   
        return {'subscriber':subscriber ,'user':user,'winner_visit_number':winner_visit_number}
...
@allow_subscribe_user
def makePaymentPage(request):
    context = {}
    return render(request, 'accounts/makePayment.html', context)


def processSubscrpition(request):
    if request.user.is_authenticated:
        user = request.user
        subscriber = Subscriber.objects.get(user=user)
        if subscriber.subscribe == False:
